...
- `今日草稿`：创建或打开今天的 Markdown，并用 Typora 打开。
- `文章列表`：查看草稿和已发布文章。
- `本地预览`：启动网站预览，并打开 `http://127.0.0.1:4321/`。
- `发布草稿`：选择草稿发布，自动整理 frontmatter、图片并运行构建检查。
- `退出`：关闭后台。如果预览服务由后台启动，也会一起关闭。

键盘操作：`Enter` 执行，`↑/↓` 或 `j/k` 移动，`r` 刷新，`q` 退出。
